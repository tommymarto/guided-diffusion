...
export EXPERIMENT_NAME="${SLURM_JOB_ID}_cifar10_cond_distributional_noweighting_without_sigma_better_channels_lambda_linear_beta_linear"
...
if [ "$LOCAL_MODE" = true ]; then
    ...
    python -m debugpy --listen 0.0.0.0:5678 --wait-for-client ./scripts/image_train.py \
        ...
        --distributional_lambda_weighting linear \
        --distributional_beta_schedule linear \
        --distributional_kernel_kwargs '{"beta": 2.0, "beta_start": 0.1}' \
